...
const capabilities = [
  {
    group: "Type-safe Scenarios",
    page: "k6 Scripts",
    desc: "Define load profiles, stages, and thresholds with auto-completion.",
    icon: TextCursorInput,
  },
  {
    group: "Playwright-like Syntax",
    page: "API Requests",
    desc: "Write HTTP requests and checks using familiar page-object patterns.",
    icon: Table2,
  },
  {
    group: "Reusable Helpers",
    page: "Load Testing",
    desc: "Helper functions for auth, parsing, and setup/teardown.",
    icon: Zap,
  },
];
  ...
  return (
    <ThreadedPageLayout sections={sections}>
      <div className="notion-page pt-0 pb-12">
        ...
        <section
          id="overview"
          className="snap-item scroll-mt-24 text-left py-12 md:py-8 border-b border-border/40 mb-12 relative"
        >
          <div className="max-w-4xl">
            ...
            <p className="text-muted-foreground text-base md:text-lg max-w-3xl mb-6 leading-relaxed">
              K6-Core brings the familiar structure, type-safety, and developer
              experience of Playwright to performance testing. Define virtual
              users, thresholds, and scenarios with zero-boilerplate setup.
        ...
        <section
          id="docs"
          className="snap-item scroll-mt-24 mb-12 relative text-left"
        >
          ...
          <DocsPage isEmbedded={true} />
        </section>

        {/* Component Matrix */}
        <section
          id="capabilities"
          className="snap-item scroll-mt-24 mb-12 relative"
        >
          <h2 className="sticky top-[60px] z-20 py-3 mb-4 backdrop-blur-md bg-background/85 text-sm font-semibold font-heading uppercase tracking-wider text-muted-foreground">
            Capabilities
          </h2>
          <SkeuCard className="overflow-hidden p-0">
            <table
              id="k6-capabilities-table"
              data-test-id="k6-capabilities-table"
              data-testid="k6-capabilities-table"
            ...
            >
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  <th
                    id="k6-hdr-group"
                    data-test-id="k6-hdr-group"
                    data-testid="k6-hdr-group"
                  ...
                  <th
                    id="k6-hdr-location"
                    data-test-id="k6-hdr-location"
                    data-testid="k6-hdr-location"
                    className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs uppercase tracking-wider"
                  >
                    Location / Usage
                  </th>
                  <th
                    id="k6-hdr-desc"
                    data-test-id="k6-hdr-desc"
                    data-testid="k6-hdr-desc"
                    className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs uppercase tracking-wider"
                  >
                    Description
                  </th>
                ...
                {capabilities.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.group}
                      id={`k6-row-${slug}`}
                      data-test-id={`k6-row-${slug}`}
                      data-testid={`k6-row-${slug}`}
                      className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors"
                    >
                      <td
                        id={`k6-cell-${slug}-group`}
                        data-test-id={`k6-cell-${slug}-group`}
                        data-testid={`k6-cell-${slug}-group`}
                      ...
                      <td
                        id={`k6-cell-${slug}-location`}
                        data-test-id={`k6-cell-${slug}-location`}
                        data-testid={`k6-cell-${slug}-location`}
                        className="px-4 py-3 text-muted-foreground"
                      >
                        {row.page}
                      </td>
                      <td
                        id={`k6-cell-${slug}-desc`}
                        data-test-id={`k6-cell-${slug}-desc`}
                        data-testid={`k6-cell-${slug}-desc`}
