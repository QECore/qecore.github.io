...
const componentMatrix = [
  {
    component: "Form Inputs & OTP",
    page: "QA Playground",
    desc: "Inputs, sliders, toggles, and 6-digit OTP fields.",
    icon: TextCursorInput,
  },
  {
    component: "Sortable Data Tables",
    page: "Projects, Tasks & QA Playground",
    desc: "Sorting, column styling, and filter search.",
    icon: Table2,
  },
  {
    component: "File Dropzones",
    page: "Documents & QA Playground",
    desc: "Drag-and-drop attachments with size checks.",
    icon: Upload,
  },
  {
    component: "Dialogs & Modals",
    page: "Projects, Tasks & Kanban Board",
    desc: "Sheet overlays, dialog popups, and date-pickers.",
    icon: SquareStack,
  },
  {
    component: "API Schema Sandbox",
    page: "Swagger Portal",
    desc: "JSON schema validation and live endpoint execution.",
    icon: BookOpen,
  },
];
      ...
      <div className="notion-page pt-0 pb-12">
            ...
            {modules.map((mod) => (
              ...
              >
                <SkeuCard className="w-full h-full transition-all duration-300 p-6 border border-border/60 group-hover:border-amber-500/40 group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:scale-[1.03] flex flex-col justify-between relative overflow-hidden">
                  <div
                    className={`absolute -top-10 -right-10 w-24 h-24 rounded-full blur-2xl pointer-events-none transition-colors ${
                      mod.color === "amber"
                  ...
                  <div className="relative z-10 flex-1 flex flex-col">
                    ...
                    <div className="mb-4">
                      <span className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/70 block mb-0.5">
                        Verify
                      </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch max-w-5xl mx-auto">
                ...
                {appArchSteps.map((step, i) => (
                  <React.Fragment key={step.label}>
                    <div className="skeu-inset px-2.5 py-3 text-center min-w-[90px] flex-1">
                      ...
                      <p className="font-heading font-semibold text-[11px] leading-tight">
                        {step.label}
                      </p>
            ...
            <SkeuCard className="p-4 flex flex-col justify-between">
              ...
              <div className="code-block text-[10.5px] p-3 flex-1 flex flex-col justify-center leading-relaxed h-full">
                ...
                <p>
                  App → <span className="text-amber-300">/api/app</span>
                </p>
                <p>
                  QA → <span className="text-amber-300">/api/auth</span>
                </p>
                <p className="mt-2 text-emerald-400">
                  {"// Backed by JSON DB → Storage"}
                </p>
              </div>
            </SkeuCard>
          </div>
        ...
        >
          ...
          <SkeuCard className="overflow-hidden p-0">
            <table
              id="landing-component-matrix-table"
              data-test-id="landing-component-matrix-table"
              data-testid="landing-component-matrix-table"
              className="w-full text-sm"
            >
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  <th
                    id="landing-hdr-group"
                    className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs uppercase tracking-wider"
                  >
                    Component Group
                  </th>
                  <th
                    id="landing-hdr-location"
                    className="text-left px-4 py-2.5 font-medium text-muted-foreground text-xs uppercase tracking-wider"
                  >
                    Page / Location
                  </th>
                ...
                {componentMatrix.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        id={`landing-cell-${slug}-location`}
                        className="px-4 py-3 text-muted-foreground"
                      >
                        {row.page}
                      </td>
                      <td
                        id={`landing-cell-${slug}-desc`}
